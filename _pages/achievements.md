...
author_profile: true
---
## Ranked 4th in Code Generation Shared Task 2025
Our team, **AdversaryAI**, participated in code generation shared. The goal of this task was to generate correct and executable Python code from natural-language instructions in Bangla.

In this work, we developed a Think–Refine–Generate (TriGen) framework that integrates LoRA-based fine-tuning, self-refinement, and unit-test-driven debugging to improve code reliability. Instead of producing code in a single shot, TriGen encourages the model to reason step by step, generate intermediate drafts, execute tests, and iteratively debug its own output. This structured reasoning loop allows the system to correct logical and syntactic errors during generation, resulting in more stable and verifiable code.

Our approach also included data augmentation and targeted reasoning prompts to help the model better understand ambiguous instructions. For final predictions, we selected outputs through systematic refinement and validation against test cases. TriGen ranked 4th on the official leaderboard.

You can find the leaderboard [here](https://noshinulfat.github.io/blp25_code_generation_task/#/home)
